...
{
    [Range(0, 8)] public float f;
    [Range(0, 3)] public float z;
    [Range(-3, 3)] public float r;
...
public static class MathUtils
{
    ...
    */
    ...
    public static (float, float) SecondOrder(float t, float f, float z, float r, float x, float prevX, float y, float dy)
    {
        (float k1, float k2, float k3) = InitK123(f, z, r);

        y = SecondOrder(t, k1, k2, k3, x, prevX, y, ref dy);
        return (y, dy);
    }
    ...
    public static float GetRotFlipY(Vector2 dir, float upDir, float rotationOffset = 0)
    {
        float rotZ = dir == Vector2.zero ? 0 : Mathf.Atan2(dir.y, dir.x) * Mathf.Rad2Deg;
        rotZ += rotationOffset;
        return (dir.x >= 0 ? rotZ : 180f - rotZ) * upDir;
    }

    public static Quaternion GetQuaternionFlipY(Vector2 dir, float upDir)
    {
        return Quaternion.Euler(upDir >= 0 ? 0 : 180, dir.x >= 0 ? 0 : 180, GetRotFlipY(dir, upDir));
    }
    #endregion
