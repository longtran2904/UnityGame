...
public partial class Enemy : MonoBehaviour
{
...
#if false
    public RangedInt numberOfEnemiesToSpawn;
                                ...
                                // Spawn wave of bullets
                                {
                                    ...
                                    // Make sure the enemy isn't inside wall
                                    {
                                        ...
                                        if (hitInfo) // NOTE: Sometime the enemy stop right before actually touching the ground (because of the raycast's length) so I will need this check.
                                        {
                                            ...
                                            ExtDebug.DrawBox(spriteOrigin - dir * (magnitude - hitInfo.distance), spriteSize / 2, Quaternion.identity, Color.white);
                                        }
                                    }
                                    ...
                                    foreach (var pos in bulletsPos)
                                    {
                                        ...
                                        pooler.SpawnFromPool<Projectile>(bullet, pos, Quaternion.Euler(euler)).Init(bulletDamage, 0, 0, true, false);
                                    }
                                }
    ...
    IEnumerator EnableTrail(float waitTime, float emitTime, bool decreaseWidthOverTime = false)
    {
        ...
        IEnumerator DecreaseTrailWidth(float decreaseTime)
        {
            ...
            {
                trail.widthMultiplier = decreaseTime / startTime;
