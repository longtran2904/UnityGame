...
public class SecondOrderDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        for (int i = 1; i < y.Length; i++)
        {
            ...
            //Debug.Log("Index: " + i + " Velocity: " + velocity);
            //x[i] = x[i - 1] + (x[i - 1] < 1 ? velocity : 0);
            /*if (i < xArr.arraySize)
                x[i] = xArr.GetArrayElementAtIndex(i).floatValue - xArr.GetArrayElementAtIndex(0).floatValue;
            else
                x[i] = x[i - 1];*/
            x[i] = i / (x.Length / 4) % 2;
        ...
        float yMax = Mathf.Max(0, Mathf.Max(y));
        ...
        Handles.BeginGUI();
        Handles.DrawLine(originP, horMaxP);
        Handles.DrawLine(ToPoint(0, yMin), ToPoint(0, yMax));
        Handles.DrawLine(verMaxP, ToPoint(tMax, 1));
        ...
        if (yMax > 5)
        {
            Handles.DrawLine(ToPoint(0, 5), ToPoint(tMax, 5));
            GUI.Label(new Rect(ToPoint(0, 5) + Vector2.left * labelSize.x + Vector2.down * labelSize.y / 2, labelSize), "5");
        }

        Handles.color = Color.green;
        for (int i = 1; i < x.Length; i++)
            Handles.DrawLine(ToPoint(dt * (i - 1), x[i - 1]), ToPoint(dt * i, x[i]));
        Handles.color = Color.cyan;
        for (int i = 1; i < y.Length; i++)
            Handles.DrawLine(ToPoint(dt * (i - 1), y[i - 1]), ToPoint(dt * i, y[i]));
