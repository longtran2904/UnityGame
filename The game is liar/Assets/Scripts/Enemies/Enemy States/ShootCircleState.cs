...
/*[CreateAssetMenu(menuName = "Enemy/States/Shoot Circle")]
public class ShootCircleState : EnemyState
{
    ...
    public override void Init(Enemies enemy)
    {
        ...
        {
            bullets[i] = ObjectPooler.instance.SpawnFromPool<Projectile>(weaponName, pos, Quaternion.identity, bulletHolder);
            bullets[i].Init(enemy.damage, 0, 0, true, false);
            bullets[i].SetVelocity(0);
            i++;
        }
        delayTime = Time.time + delayBulletTime;
        base.Init(enemy);
    }
