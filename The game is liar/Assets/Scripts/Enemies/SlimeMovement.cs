...
public class SlimeMovement : EnemiesMovement
{
    ...
    void JumpToPlayer()
    {
        if (Time.time > timeBtwJumpsValue)
        {
            ...
            float fullDistanceX = Vector2.one.normalized.x * speed;
            float currentSpeed = Mathf.Lerp(0, speed, distanceX / fullDistanceX);
            InternalDebug.Log("Current speed: " + currentSpeed);
            rb.velocity = new Vector2(MathUtils.Signed(distanceX), 1).normalized * speed;
            InternalDebug.Log("Velocity: " + rb.velocity);
